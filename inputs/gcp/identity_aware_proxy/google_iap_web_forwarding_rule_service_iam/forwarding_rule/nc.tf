...
resource "google_iap_web_forwarding_rule_service_iam_member" "nc4" {
  project                      = "nc_admin"
  forwarding_rule_service_name = "grafana-be-prod"
  role                         = "nc4"
  member                       = "user:jane@example.com"
}

resource "google_iap_web_forwarding_rule_service_iam_member" "nc5" {
  project                      = "nc_env"
  forwarding_rule_service_name = "orders-be-demo"
  role                         = "nc5"
  member                       = "user:jane@example.com"
}
